...
const (
	...
	TablesQuery = `SELECT tb.table_name, ordinal_position, column_name, data_type FROM information_schema.tables tb 
	...
	PrimaryKeysQuery = `SELECT tc.table_name,column_name FROM information_schema.table_constraints  tc 
		...
		where tc.table_schema = public and constraint_type = PRIMARY KEYand tc.table_catalog = $1;`
	ForeignKeysQuery = `select tc.constraint_name, tc.table_name, kcu.column_name, ccu.table_name AS foreign_table_name, 
		...
		where tc.table_schema = publi and constraint_type = FOREIGN KEY and tc.table_catalog = $1;`
